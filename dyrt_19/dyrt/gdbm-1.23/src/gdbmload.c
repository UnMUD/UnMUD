...
get_dump_line(struct dump_file *file)
{
  ...
  if (file->lblevel == 0)
  {
    while (fgets(buf, sizeof buf, file->fp))
    {
      ...
      {
        size_t s = ((file->lblevel + n + _GDBM_MAX_DUMP_LINE_LEN) / _GDBM_MAX_DUMP_LINE_LEN) * _GDBM_MAX_DUMP_LINE_LEN;
...
get_data(struct dump_file *file)
{
  ...
  while ((n = get_dump_line(file)))
  {
    ...
    {
      size_t s = ((file->buflevel + n + _GDBM_MAX_DUMP_LINE_LEN - 1) / _GDBM_MAX_DUMP_LINE_LEN) * _GDBM_MAX_DUMP_LINE_LEN;
...
get_parms(struct dump_file *file)
{
  ...
  while ((n = get_dump_line(file)))
  {
    ...
    {
      size_t s = ((file->buflevel + n + _GDBM_MAX_DUMP_LINE_LEN) / _GDBM_MAX_DUMP_LINE_LEN) * _GDBM_MAX_DUMP_LINE_LEN;
      char *newp = realloc(file->buffer, s);
      if (!newp)
        return GDBM_MALLOC_ERROR;
      file->buffer = newp;
...
int gdbm_load_from_file(GDBM_FILE *pdbf, FILE *fp, int replace,
                        int meta_mask,
                        unsigned long *line)
{
  struct dump_file df;
...
int gdbm_load(GDBM_FILE *pdbf, const char *filename, int replace,
              int meta_mask,
              unsigned long *line)
{
  FILE *fp;
