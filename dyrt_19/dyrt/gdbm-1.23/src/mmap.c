...
#define _GDBM_IN_MAPPED_REGION_P(dbf, off) \
  ((off) >= (dbf)->mapped_off && ((off) - (dbf)->mapped_off) < (dbf)->mapped_size)
...
{
  if (delta >= 0 && off_t_sum_ok(dbf->mapped_off, dbf->mapped_size) && off_t_sum_ok(dbf->mapped_off + dbf->mapped_size, delta))
    return dbf->mapped_off + dbf->mapped_size + delta;
  return -1;
}

/* Unmap the region. Reset all mapped fields to initial values. */
void _gdbm_mapped_unmap(GDBM_FILE dbf)
{
...
int _gdbm_internal_remap(GDBM_FILE dbf, size_t size)
{
  void *p;
...
int _gdbm_mapped_remap(GDBM_FILE dbf, off_t size, int flag)
{
  off_t file_size, pos;
...
/* Initialize mapping system. If the file size is less than MAPPED_SIZE_MAX,
   ...
   bytes. */
int _gdbm_mapped_init(GDBM_FILE dbf)
{
...
off_t _gdbm_mapped_lseek(GDBM_FILE dbf, off_t offset, int whence)
{
  if (dbf->memory_mapping)
  {
    off_t needle;
...
int _gdbm_mapped_sync(GDBM_FILE dbf)
{
  int rc;
