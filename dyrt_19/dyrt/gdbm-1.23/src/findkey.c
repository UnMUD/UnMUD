...
int _gdbm_findkey(GDBM_FILE dbf, datum key, char **ret_dptr, int *ret_hash_val)
{
  ...
  if (dbf->cache_mru->ca_data.elem_loc != -1 && new_hash_val == dbf->cache_mru->ca_data.hash_val && dbf->cache_mru->ca_data.key_size == key.dsize && dbf->cache_mru->ca_data.dptr != NULL && memcmp(dbf->cache_mru->ca_data.dptr, key.dptr, key.dsize) == 0)
  {
  ...
  {
    key_size = dbf->bucket->h_table[elem_loc].key_size;
    if (bucket_hash_val != new_hash_val || key_size != key.dsize || memcmp(dbf->bucket->h_table[elem_loc].key_start, key.dptr, (SMALL < key_size ? SMALL : key_size)) != 0)
    {
