...
off_t _gdbm_alloc(GDBM_FILE dbf, int num_bytes)
{
 ...
 something on the stack. */
    if ((dbf->avail->count <= (dbf->avail->size >> 1)) && (dbf->avail->next_block != 0))
...
int _gdbm_free(GDBM_FILE dbf, off_t file_adr, int num_bytes)
{
  avail_elem temp;
...
pop_avail_block(GDBM_FILE dbf)
{
  ...
  new_el.av_size = (((dbf->avail->size * sizeof(avail_elem)) >> 1) + sizeof(avail_block));
...
push_avail_block(GDBM_FILE dbf)
{
  ...
  av_size = ((dbf->avail->size * sizeof(avail_elem)) >> 1) + sizeof(avail_block);
...
void _gdbm_put_av_elem(avail_elem new_el, avail_elem av_table[], int *av_count,
                       int can_merge)
{
  int index;
...
adjust_bucket_avail(GDBM_FILE dbf)
{
  ...
  while (dbf->bucket->av_count > BUCKET_AVAIL - third && dbf->avail->count < dbf->avail->size)
  {
