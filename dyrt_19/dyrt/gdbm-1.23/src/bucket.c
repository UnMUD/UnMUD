...
void _gdbm_new_bucket(GDBM_FILE dbf, hash_bucket *bucket, int bits)
{
  int index;
 ...
 */
int _gdbm_get_bucket(GDBM_FILE dbf, int dir_index)
{
  int rc;
  ...
  case cache_new:
    ...
    if (!(bucket->count >= 0 && bucket->count <= dbf->header->bucket_elems && bucket->bucket_bits >= 0 && bucket->bucket_bits <= dbf->header->dir_bits))
    {
...
int _gdbm_split_bucket(GDBM_FILE dbf, int next_insert)
{
  ...
  while (dbf->bucket->count == dbf->header->bucket_elems)
  {
    ...
    newcache[1]->ca_bucket->bucket_avail[0].av_adr = _gdbm_alloc(dbf, dbf->header->block_size);
    if (newcache[1]->ca_bucket->bucket_avail[0].av_adr == 0)
      return -1;
    newcache[1]->ca_bucket->bucket_avail[0].av_size = dbf->header->block_size;
    ...
    {
      newcache[0]->ca_bucket->bucket_avail[index1++] = dbf->bucket->bucket_avail[index];
    }
...
int _gdbm_write_bucket(GDBM_FILE dbf, cache_elem *ca_entry)
{
  int rc;
...
int _gdbm_cache_init(GDBM_FILE dbf, size_t size)
{
  int bits;
  int cache_auto;
