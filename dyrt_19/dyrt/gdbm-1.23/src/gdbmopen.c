...
validate_header_std(gdbm_file_header const *hdr, struct stat const *st)
{
  ...
  if (!(hdr->block_size > 0 && hdr->block_size > sizeof(gdbm_file_header) && hdr->block_size - sizeof(gdbm_file_header) >= sizeof(avail_block)))
  {
  ...
  if (!(hdr->dir > 0 && hdr->dir < st->st_size && hdr->dir_size > 0 && hdr->dir + hdr->dir_size < st->st_size))
...
validate_header_numsync(gdbm_file_header const *hdr, struct stat const *st)
{
  ...
  if (!(hdr->block_size > 0 && hdr->block_size > (sizeof(gdbm_file_header) + sizeof(gdbm_ext_header)) && hdr->block_size - (sizeof(gdbm_file_header) + sizeof(gdbm_ext_header)) >= sizeof(avail_block)))
  {
    return GDBM_BLOCK_SIZE_ERROR;
  }
             ...
             int flags, void (*fatal_func)(const char *))
{
  ...
  if (file_stat.st_size == 0)
  {
    ...
    dbf->avail->size = (dbf->avail_size - offsetof(avail_block, av_table)) / sizeof(avail_elem);
    dbf->avail->count = 0;
...
_gdbm_convert_to_numsync(GDBM_FILE dbf)
{
   ...
   */
  old_avail->size = (dbf->avail_size - offsetof(avail_block, av_table)) / sizeof(avail_elem);
...
int gdbm_convert(GDBM_FILE dbf, int flag)
{
  int rc;
