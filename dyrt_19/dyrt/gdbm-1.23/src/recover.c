...
check_db(GDBM_FILE dbf)
{
       ...
       bucket_dir = _gdbm_next_bucket_dir(dbf, bucket_dir))
  {
    ...
    {
      if (dbf->bucket->count < 0 || dbf->bucket->count > dbf->header->bucket_elems)
        return 1;
...
run_recovery(GDBM_FILE dbf, GDBM_FILE new_dbf, gdbm_recovery *rcvr, int flags)
{
       ...
       bucket_dir = _gdbm_next_bucket_dir(dbf, bucket_dir))
  {
    ...
    if (_gdbm_get_bucket(dbf, bucket_dir))
    {
      ...
      rcvr->failed_buckets++;
      if ((flags & GDBM_RCVR_MAX_FAILED_BUCKETS) && rcvr->failed_buckets == rcvr->max_failed_buckets)
        return -1;
      if ((flags & GDBM_RCVR_MAX_FAILURES) && (rcvr->failed_buckets + rcvr->failed_keys) == rcvr->max_failures)
        return -1;
    }
    else
    {
        ...
        else
        {
          if (flags & GDBM_RCVR_ERRFUN)
            rcvr->errfun(rcvr->data,
                         ...
                         dbf->bucket->h_table[i].key_size + dbf->bucket->h_table[i].data_size,
                         gdbm_db_strerror(dbf));
          rcvr->failed_keys++;
          if ((flags & GDBM_RCVR_MAX_FAILED_KEYS) && rcvr->failed_keys == rcvr->max_failed_keys)
            return -1;
          if ((flags & GDBM_RCVR_MAX_FAILURES) && (rcvr->failed_buckets + rcvr->failed_keys) == rcvr->max_failures)
            return -1;
          continue;
...
int gdbm_recover(GDBM_FILE dbf, gdbm_recovery *rcvr, int flags)
{
  ...
  if ((flags & GDBM_RCVR_FORCE) || check_db(dbf))
  {
    ...
    new_dbf = gdbm_fd_open(fd, new_name, dbf->header->block_size,
                           GDBM_WRCREAT | (dbf->cloexec ? GDBM_CLOEXEC : 0) | (dbf->xheader ? GDBM_NUMSYNC : 0) | GDBM_CLOERROR, dbf->fatal_err);
