...
static struct snapshot_status_info snapshot_status_info[] = {
    ...
    [GDBM_SNAPSHOT_BAD] = {"GDBM_SNAPSHOT_BAD", N_("Neither snapshot is readable"), snapshot_print_fn},
    [GDBM_SNAPSHOT_ERR] = {"GDBM_SNAPSHOT_ERR", N_("Error selecting snapshot"), snapshot_err_fn},
    [GDBM_SNAPSHOT_SAME] = {"GDBM_SNAPSHOT_SAME", N_("Snapshot modes and dates are the same"), snapshot_print_fn},
    [GDBM_SNAPSHOT_SUSPICIOUS] = {"GDBM_SNAPSHOT_SUSPICIOUS", N_("Snapshot sync counters differ by more than 1"), snapshot_print_fn}};
...
static struct command command_tab[] = {
    ...
    {
        .name = "export",
        .args = {{N_("FILE"), GDBM_ARG_STRING}, {"[truncate]", GDBM_ARG_STRING}, {"[binary|ascii]", GDBM_ARG_STRING}, {NULL}},
        .doc = N_("export"),
        .tok = T_CMD,
        .begin = checkdb_begin,
    ...
    {
        .name = "fetch",
        .args = {{N_("KEY"), GDBM_ARG_DATUM, DS_KEY}, {NULL}},
        .doc = N_("fetch record"),
        .tok = T_CMD,
        .begin = checkdb_begin,
    ...
    {
        .name = "import",
        .args = {{N_("FILE"), GDBM_ARG_STRING}, {"[replace]", GDBM_ARG_STRING}, {"[nometa]", GDBM_ARG_STRING}, {NULL}},
        .doc = N_("import"),
        .tok = T_CMD,
    ...
    {
        .name = "store",
        .args = {{N_("KEY"), GDBM_ARG_DATUM, DS_KEY}, {N_("DATA"), GDBM_ARG_DATUM, DS_CONTENT}, {NULL}},
        .doc = N_("store"),
        .tok = T_CMD,
        .begin = checkdb_begin,
    ...
    {
        .name = "recover",
        .args = {{"[verbose]", GDBM_ARG_STRING}, {"[summary]", GDBM_ARG_STRING}, {"[backup]", GDBM_ARG_STRING}, {"[force]", GDBM_ARG_STRING}, {"[max-failed-keys=N]", GDBM_ARG_STRING}, {"[max-failed-buckets=N]", GDBM_ARG_STRING}, {"[max-failures=N]", GDBM_ARG_STRING}, {NULL}},
    ...
    {
        .name = "bucket",
        .args = {{N_("[NUMBER]"), GDBM_ARG_STRING}, {NULL}},
    ...
    {
        .name = "snapshot",
        .args = {{"FILE", GDBM_ARG_STRING}, {"FILE", GDBM_ARG_STRING}, {NULL}},
    ...
    {
        .name = "define",
        .args = {{"key|content", GDBM_ARG_STRING}, {"{ FIELD-LIST }", GDBM_ARG_STRING}, {NULL}},
    ...
    {
        .name = "source",
        .args = {{"FILE", GDBM_ARG_STRING}, {NULL}},
    ...
    {
        .name = "history",
        .args = {{N_("[FROM]"), GDBM_ARG_STRING}, {N_("[COUNT]"), GDBM_ARG_STRING}, {NULL}},
    ...
    {
        .name = "perror",
        .args = {{"[CODE]", GDBM_ARG_STRING}, {NULL}},
...
int command_lookup(const char *str, struct locus *loc, struct command **pcmd)
{
  ...
  if (state == fcom_init)
    lerror(loc, interactive() ? _("Invalid command. Try ? for help.") : _("Unknown command"));
  if (!found)
    return T_BOGUS;
...
void gdbmarglist_init(struct gdbmarglist *lst, struct gdbmarg *arg)
{
  if (arg)
    arg->next = NULL;
...
void gdbmarglist_add(struct gdbmarglist *lst, struct gdbmarg *arg)
{
  arg->next = NULL;
  if (lst->tail)
    lst->tail->next = arg;
...
void timing_stop(struct timing *t)
{
  struct rusage r;
  struct timeval now;
