...
int s_char(struct xdatum *xd, char *str)
{
  xd_store(xd, str, 1);
  return 0;
}
...
#define DEFNSCAN(name, type, temptype, strto) \
  int name(struct xdatum *xd, char *str)      \
  {                                           \
    temptype n;                               \
    type t;                                   \
    char *p;                                  \
...
int s_double(struct xdatum *xd, char *str)
{
  double d;
  char *p;
...
int s_float(struct xdatum *xd, char *str)
{
  float d;
  char *p;
...
int s_stringz(struct xdatum *xd, char *str)
{
  xd_store(xd, str, strlen(str) + 1);
  return 0;
}

int s_string(struct xdatum *xd, char *str)
{
        ...
        for (i = 0; i < ds->v.field.dim; i++)
        {
          ...
          else
          {
            n = ds->v.field.type->format(fp,
                                         (char *)dat->dptr + off,
                                         ds->v.field.type->size ? ds->v.field.type->size : dat->dsize - off);
            off += n;
          }
        }
      }
