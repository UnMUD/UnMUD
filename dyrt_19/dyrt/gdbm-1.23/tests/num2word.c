...
void copy(const char *str, int dch)
{
...
void format_1000(numeral_t num, int more)
{
  size_t n = num % 100;
  num /= 100;
  format_100(n);
...
void format_number(numeral_t num)
{
  int s = 0;
  size_t off;
...
void print_number(numeral_t num)
{
  format_number(num);
  if (revert_option)
    printf("%s\t%lu\n", buffer + bufoff, num);
...
void print_range(numeral_t num, numeral_t to)
{
  for (; num <= to; num++)
    print_number(num);
}
