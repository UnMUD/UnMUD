...
void throwcom(void)
{
  ...
  if ((ob1 == OBJ_NIBELUNG_ARROW) || (ob1 == OBJ_NIBELUNG_ARROW2) || (ob1 == OBJ_NIBELUNG_ARROW3))
  {
    if (ploc(mynum) == oloc(OBJ_NIBELUNG_HOLE))
    {
      if (((state(OBJ_NIBELUNG_FIRE) == 1) && (ob1 == OBJ_NIBELUNG_ARROW)) || ((state(OBJ_NIBELUNG_FIRE) == 2) && (ob1 == OBJ_NIBELUNG_ARROW2)) || ((state(OBJ_NIBELUNG_FIRE) == 3) && (ob1 == OBJ_NIBELUNG_ARROW3)))
      {
...
void wherecom()
{
  ...
  for (cha = 0; cha < numobs; cha++)
  {
    ...
    if (EQ(oname(cha), item1) || (plev(mynum) >= LVL_APPREN && EQ(oaltname(cha), item1)))
    {
      num_obj_found++;
      if (plev(mynum) >= LVL_APPREN)
        bprintf("[%3d] ", cha);
...
void examcom(void)
{
  ...
  case OBJ_NIBELUNG_FIRE:
    if (iscarrby(OBJ_NIBELUNG_FIRESTONE, mynum) && (state(OBJ_NIBELUNG_FIRE) == 0))
    {
...
void emptycom(void)
{
  ...
  else if ((otstbit(a, OFL_OPENABLE) || otstbit(a, OFL_LOCKABLE)) && state(a) > 0)
  {
...
void wavecom(void)
{
  ...
  case OBJ_WINDOW_BRIDGE_WAND:
    if ((state(OBJ_WINDOW_BRIDGE_FIRE) == 1) && (oloc(OBJ_WINDOW_BRIDGE_FIRE) == ploc(mynum)))
    {
