...
{
  return onum(obj) == OBJ_TREEHOUSE_SHIELD ||
...
{
  return onum(obj) == OBJ_LABYRINTH_MASK || onum(obj) == OBJ_ANCIENT_WARMASK;
}
...
void putcom(void)
{
  ...
  if (otstbit(a, OFL_GETFLIPS))
    setobjstate(a, 0);
  if (ploc(mynum) == LOC_TOWER_TREASURE && state(OBJ_TOWER_DOOR_TREASURE) == 0 && ishere(a))
  {
...
void aobjsat(int loc, int mode, int marg)
{
  ...
  int obj;
  int stp;
  int_set *inv = mode == IN_ROOM ? linv(loc) : mode == IN_CONTAINER ? oinv(loc)
                                                                    : pinv(loc);
  ...
  for (obj = first_obj(inv); obj != SET_END; obj = next_obj(inv))
  {
    ...
    if ((mode == CARRIED_BY && iscarrby(obj, loc)) || (mode == IN_CONTAINER && iscontin(obj, loc)))
    {
      ...
      *s++ = ' ';
      ...
      if (mode == CARRIED_BY)
      {
        ...
        if (pwpn(loc) == obj && ocarrf(obj) == WIELDED_BY)
        {
          if (wwl)
            *s++ = ',';
          else
...
static int fobnsys(char *name, int ctrl, int ct_inf, int_set *inv)
{
  ...
  *p = '\0';
  ...
  if (ctrl == 0)
  { /* Look for first object with this name*/

    for (obj = 0; obj < numobs; obj++)
    {
      n = EQ(b, oname(obj)) ? oname(obj) : EQ(b, oaltname(obj)) ? oaltname(obj)
                                                                : NULL;
...
int find_object_by_id(long int id)
{
  ...
  return (x = lookup_entry(id, &id_table)) == NOT_IN_TABLE || x < 0 || x >= numobs ? -1 : x;
}
...
int get1objfrom(int ob, int container)
{
  ...
  if (ob == OBJ_WINDOW_SHIELD)
  {
    ...
    else if (ishere(OBJ_WINDOW_SHIELD2))
...
int getcom(void)
{
  ...
  if (brkword() != -1 && (EQ(wordbuf, "from") || EQ(wordbuf, "out")))
  {
    ...
    if ((otstbit(container, OFL_OPENABLE) || otstbit(container, OFL_LOCKABLE)) && state(container) > 0)
    {
...
Boolean p_ishere(int plr, int item)
{
  ...
  if (ocarrf(item) != IN_ROOM || oloc(item) != ploc(plr) || ploc(plr) == 0)
    return False;
...
int dropobj(int obj)
{
  ...
  if (a == OBJ_CATACOMB_CUPSERAPH && ploc((max_players + MOB_CATACOMB_SERAPH)) == LOC_START_CHURCH)
  ...
  /* OAKTREE */
  if ((l >= LOC_OAKTREE_MAGNOLIA && l <= LOC_OAKTREE_TREE1) || l == LOC_OAKTREE_ILEX || (l >= LOC_OAKTREE_WALNUT && l <= LOC_OAKTREE_FIG))
  {
...
Boolean set_weapon(int plr, int wpn)
{
  ...
  if (wpn < 0 || odamage(wpn) == 0 || ocarrf(wpn) < CARRIED_BY || oloc(wpn) != plr)
  {
...
{
  if (state >= 0 && state <= omaxstate(obj) && (olinked(obj) == -1 || state <= omaxstate(olinked(obj))))
  {
...
Boolean p_ohany(int plr, int mask)
{
  ...
  for (i = 0; i < lnumobs(ploc(plr)); i++)
  {
    ...
    if (p_ishere(plr, a) && (obits(a) & mask))
      return True;
  }
  ...
  for (i = 0; i < pnumobs(plr); i++)
  {
    ...
    if (iscarrby(a, plr) && (obits(a) & mask))
      return True;
  }
