...
void pocketcom(void)
{
  ...
  else
  {
    ...
    {
      if (strcasecmp(roomname, sdesc(pocketloc)))
        continue;
      setup_globals(x);
...
void wizlock()
{
  ...
  if (strncasecmp(wordbuf, "Off", y) == 0 || strncasecmp(wordbuf, "Unlock", y) == 0)
  {
    n = 0;
  }
...
void socinfo(void)
{
  ...
  for (i = 0; i < max_players; i++)
  {
    ...
    bprintf("&+Y %-*s", PNAME_LEN, EMPTY(pname(i)) ? "Undecided" : pname(i));
...
void killsocket(void)
{
  ...
  bprintf("Socket chosen: %d. Trying to clean up socket...\n"
          "Socket killed for %s.\n",
          s, EMPTY(pname(s)) ? "John Doe" : pname(s));

  mudlog("SOCKET: Socket %d killed for %s by %s.", s, EMPTY(pname(s)) ? "John Doe" : pname(s), pname(mynum));
