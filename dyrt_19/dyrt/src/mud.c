...
void talker(void)
{
  ...
  if (ptstflg(mynum, PFL_CLONE) && ((k = get_zone_by_name(pname(mynum))) < 0 || ztemporary(k)))
  {

    load_zone(pname(mynum), NULL, NULL, NULL, NULL, NULL, NULL);
  }

  setploc(mynum, exists(k = find_loc_by_id(phome(mynum))) ? k : randperc() > 50 ? LOC_START_TEMPLE
                                                                                : LOC_START_CHURCH);
...
static void get_yn_handler(char *yn)
{
  ...
  if (yn == NULL)
  {
    ...
    switch (cur_player->yn_handler.assume)
    {
    case ASSUME_YES:
