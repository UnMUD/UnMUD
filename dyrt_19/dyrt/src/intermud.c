...
static int mudFind(const void *key, const void *mud)
{
  ...
  minLen = (strlen(find) < strlen(mudinfo->name)) ? strlen(find) : strlen(mudinfo->name);
...
int boot_intermud(FILE *fp, char *fname)
{
  ...
  while (!feof(fp))
  {
    ...
    else if (EQ(opt, "routerip"))
    {
      raddr = COPY(value);
    }
    else if (EQ(opt, "routername"))
    {
      rname = COPY(value);
    }
    else if (EQ(opt, "routerport"))
    {
      rport = atoi(value);
    }
    else if (EQ(opt, "player port"))
    {
      pport = atoi(value);
    }
    else if (EQ(opt, "tcp port"))
    {
      tport = atoi(value);
    }
    else if (EQ(opt, "udp port"))
    {
      uport = atoi(value);
    }
    else if (EQ(opt, "mudlib"))
    {
      mudlib = COPY(value);
    }
    else if (EQ(opt, "basemudlib"))
    {
      basemudlib = COPY(value);
    }
    else if (EQ(opt, "driver"))
    {
      driver = COPY(value);
    }
    else if (EQ(opt, "email"))
    {
      adminEmail = COPY(value);
    }
    else if (EQ(opt, "mudType"))
    {
      mudType = COPY(value);
    }
    else if (EQ(opt, "openStatus"))
    {
      openStatus = COPY(value);
    }
  }
...
void parseList2(listObject *list)
{
  ...
  if (list->origString[0] == '"')
  {
    ...
    for (; *p && (*p != '"'); p++)
      if (*p == '\\')
        p++; /* Step past the next char, don't care what it is now */
...
void destroyMapping(mappingObject *map)
{
  ...
  for (x = 0; x < map->numElements; x++)
  {
    ...
    {
    case TYPE_LIST:
      destroyList((listObject *)map->keys[x]);
      break;
    case TYPE_MAPPING:
      destroyMapping((mappingObject *)map->keys[x]);
      break;
    case TYPE_STRING:
      if (map->keys[x])
        free(map->keys[x]);
    ...
    {
    case TYPE_LIST:
      destroyList((listObject *)map->values[x]);
      break;
    case TYPE_MAPPING:
      destroyMapping((mappingObject *)map->values[x]);
      break;
    case TYPE_STRING:
      if (map->values[x])
        free(map->values[x]);
...
DECLARE_HANDLER(_imWhoReq)
{
  ...
  for (i = 0, num = 0; i < max_players; i++)
  {
    ...
    if (EMPTY(pname(i)) || !players[i].iamon || pstr(i) < 0 || !ptstflg(i, PFL_INTERMUD) /* Can't access, can't be seen */
...
DECLARE_HANDLER(_imFingerReq)
{
  ...
  if ((b = fpbns(packet->data[6])) != -1 && pvis(b) == 0 && b < max_players && ptstflg(b, PFL_INTERMUD) && !ststflg(b, SFL_INTERVIS))
  {
    in_file = False;
  ...
  if (plev(b) >= LVL_GOD)
    sprintf(level, "God");
  else if (plev(b) >= LVL_DEMI)
    sprintf(level, "Demi-God");
  else if (plev(b) >= LVL_ARCHWIZARD)
    sprintf(level, "Arch Wizard");
  else if (plev(b) >= LVL_WIZARD)
    sprintf(level, "Wizard");
  else if (plev(b) > 0)
  {
    if (in_file)
      sprintf(level, "%s", ((xtstbit(d.p_sflags, SFL_FEMALE)) ? FLevels : MLevels)[plev(b)]);
    else
      sprintf(level, "%s", ((ststflg(b, SFL_FEMALE)) ? FLevels : MLevels)[plev(b)]);
  }
  else
    sprintf(level, "Mobile");
...
DECLARE_HANDLER(_imFingerReply)
{
  if (packet->type[5] == TYPE_STRING)
  {
    ...
    if (types[6] == TYPE_STRING)
      name = data[6];
    else
      name = "Unknown";
    if (types[7] == TYPE_STRING)
      title = data[7];
    else
      title = "";
    if (types[8] == TYPE_STRING)
      realName = data[8];
    else
      realName = "";
    if (types[9] == TYPE_STRING)
      email = data[9];
    else
      email = "Confidential";
    if (types[10] == TYPE_STRING)
      loggedOn = data[10];
    else
      loggedOn = "Unknown";
    if (types[11] == TYPE_INT)
      idle = (int)data[11];
    else
      idle = 0;
    if (types[12] == TYPE_STRING)
      where = data[12];
    else
      where = "Unknown";
    if (types[13] == TYPE_STRING)
      level = data[13];
    else
      level = "";
    if (types[15] == TYPE_STRING)
      misc = data[14];
    else
      misc = "";
