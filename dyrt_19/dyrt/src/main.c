...
static int xmain_reboot(int fd)
{
  ...
  while (f)
  {
    ...
    if (new_loc == -1)
      new_loc = (phome(mynum) < 0) ? phome(mynum) : ((randperc() > 66) ? LOC_START_TEMPLE : ((randperc() > 33) ? LOC_START_CHURCH : LOC_HAVEN_HAVEN));
    setpwpn(mynum, -1);
    setphelping(mynum, -1);
...
void delaycom()
{
  ...
  else
  {
    ...
    mudlog("&+bDelayed Reboot: &+r%s&+b by &+Y%s", the_world->w_delayed ? "ENABLED" : "DISABLED", pname(mynum));
  }
}
