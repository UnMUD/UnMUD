...
#ifdef INTERMUD
extern int boot_intermud(FILE *f, char *fname);
#endif
/*
...
int load_locations(int zone, FILE *f, int *loaded, int *infile)
{
       ...
       ct < numloc + num_ld; ct++, r++)
  {
    ...
    fscanf(f, "0x%8lx", &r->r_flags_reset);
    get_newline(f);
    ...
    mem_used += get_set_mem_usage(&r->objects) + get_set_mem_usage(&r->mobiles) + get_set_mem_usage(&r->exits_to_me);
  }
