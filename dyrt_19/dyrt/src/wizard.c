...
void syslog_prompt(char *input)
{
  ...
  if (*input == 'q' || *input == 'Q')
  {
    if (cur_player->syslog_popened)
      pclose(cur_player->syslog_fp);
    else
      fclose(cur_player->syslog_fp);
    get_command(NULL);
    return;
  }
  if (*input == '*')
...
void filehandle(char *filename, int mode)
{
  ...
  if (lines >= 22)
  {
    ...
    bprintf("Press [RETURN] to continue or 'q' to quit...");
    if (endl)
      cur_player->syslog_popened = True;
    else
      cur_player->syslog_popened = False;
...
void setcom()
{
  ...
  if (brkword() == -1 || ((is_oflag = (p = tlookup(wordbuf, Props)) == -1) && (p = tlookup(wordbuf, Oflags)) == -1))
  {
  ...
  if ((is_oflag || p <= SET_TEXT_MIN || p > SET_TEXT_MAX) && brkword() == -1)
  {
  ...
  if (is_oflag)
  {
    ...
    if ((v = tlookup(wordbuf, OO)) == -1 && (v = tlookup(wordbuf, TF)) == -1)
    {
  ...
  else if (p <= SET_TEXT_MIN || p > SET_TEXT_MAX)
  {
    ...
    case SET_BVALUE:
      if (v > O_BVALUE_MAX ||
          (v > obaseval(o) && v > 400 && !ptstflg(mynum, PFL_FROB)))
        limit = O_BVALUE_MAX;
      else
        osetbaseval(o, v);
      break;
...
void tracecom()
{
  ...
  if (cur_player->tr.trace_item != -1)
  {
    bprintf("Stopped tracing %s.\n",
            cur_player->tr.trace_class == 1 ? oname(cur_player->tr.trace_item) : pname(cur_player->tr.trace_item));
...
void bancom(Boolean hosts)
{
  ...
  for (i = 0; i < num; i++)
    if (EQ(names[i], foo))
    {
      ...
      for (j = 0; j < num; j++)
        if (j != i && !EMPTY(names[j]))
          fprintf(ban_file, "%s\n", names[j]);
      bprintf("Removed `%s' from banned %s file.\n", names[i],
              hosts ? "hosts" : "characters");
      mudlog("%s un-banned %s %s", pname(mynum), hosts ? "host" : "character", foo);
      fclose(ban_file);
      return;
    }
  ...
  bprintf("Added `%s' to banned %s file.\n", foo,
          hosts ? "hosts" : "characters");
  mudlog("%s banned %s %s", pname(mynum), hosts ? "host" : "character", foo);
  fclose(ban_file);
}
