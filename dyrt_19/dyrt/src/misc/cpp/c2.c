...
{ /* process '#define' */
	...
	np = slookup(pin, p, 1);
	if (oldval = np->value)
		free(lastcopy); /* was previously defined */
	b = 1;
	cf = pin;
	while (cf < p)
	{ /* update macbit */
	...
	*psav++ = '\0';
	...
	{		  /* redefinition */
		...
		{ /* redefinition different from old */
			...
			free(space); /* identical redef.; reclaim space */
	}
	else
		np->value = psav - 1;
	--flslvl;
...
{ /* kluge */
	...
	cinit = SALT;
	sp = stsym(s);
	name = malloc(strlen(sp->name) + 1 + 1);
	name[0] = '#';
	strcpy(name + 1, sp->name);
	sp->name = name;
	cinit = 0;
	return (sp);
}
