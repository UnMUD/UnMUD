...
template <typename type> inline void insert(std::ostream &s, const type &t) {
  s << t;
}
...
template <> inline void insert<sint64>(std::ostream &s, const sint64 &n) {
  char digits[21] = "01234567890123456789";
  s << _i64toa(n, digits, 10);
}

template <> inline void insert<uint64>(std::ostream &s, const uint64 &n) {
  char digits[21] = "01234567890123456789";
  s << _ui64toa(n, digits, 10);
}

template <> inline sint64 &extract<sint64>(std::istream &s, sint64 &n) {
...
template <> inline uint64 &extract<uint64>(std::istream &s, uint64 &n) {
  // skip whitespace if needed
  if (s.flags() & std::ios::skipws)
    s >> std::ws;
...
template <typename type> inline type totype(const std::string &p_string) {
