...
inline iterator double_find_if(iterator begin, iterator end, firstpass one,
                               secondpass two) {
...
inline function operate_on_if(iterator begin, iterator end, function func,
                              qualify q) {
  while (begin != end) {
    if (q(*begin))
      func(*begin);
...
template <class datatype> struct always {
