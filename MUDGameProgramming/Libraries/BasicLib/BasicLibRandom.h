...
struct linear_congruency {
  linear_congruency(type p_seed = 0) : m_seed(p_seed) { /* do nothing */
  }
...
// typedef linear_congruency<uint64, 18446744073709551615, 6364136223846793005,
// 12345> randon64;
...
struct normal_generator {
  ...
  double operator()() {
    ...
    return m_rho * (m_valid ? cos(2 * pi * m_rho1) : sin(2 * pi * m_rho1)) *
               m_sigma +
           m_mean;
  }
...
struct random_range {
  ...
  double operator()() { return generate(m_range, m_offset); }

  double operator()(double a, double b) { return generate(b - a, a); }
...
struct random_int_range {
  ...
  sint32 operator()(sint32 a, sint32 b) { return generate(b - a + 1, a); }
...
struct simulated_binomial {
  ...
  sint32 operator()(sint32 a, sint32 b) { return generate(b - a, a); }
