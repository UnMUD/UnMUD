...
int num_levels[] = { 1, LVL_WIZARD, LVL_WIZARD + 1, LVL_ARCHWIZARD,
		   LVL_DEMI, LVL_GOD };
...
static Boolean ok()
{
  ...
  case 'W': return (l >= LVL_WIZARD + 1);
  case 'X': return (l >= LVL_WIZARD);
  case 'L':
    if (*s == '-' || isdigit(*s)) {
      k = strtol(s,&t,10);
      s = t;
  ...
  case 'M':
	...
	*t++ = *s++;
	...
	exit(1);
      ...
      return test_bit(&m,k);
  case 'Z': return (p.l == 0 && p.h == 0);
  case 'Y': return (m.h == 0 && m.h == 0);
  default:
    if (!isalpha(*s))
      return True;
    fprintf(stderr, "\nIllegal code, %c at %s\n", s[-1], s - 1);
...
int main(int argc, char **argv)
{
  ...
  l = atoi(argv[1]);
  sscanf(argv[2], "0x%8x:0x%8x", &(p.h), &(p.l));
  sscanf(argv[3], "0x%8x:0x%8x", &(m.h), &(m.l));
  if (argc < 5)
    filter(stdin);
