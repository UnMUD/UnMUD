...
{ 
  ...
  "description", "armor",      "wimpy",  TABLE_END 
};
...
void (*ChFunTable[])(void) = {
  ...
  change_desc, change_armor, change_wimpy
};
...
static void change_sex(void)
{
    ...
    if (!ptstflg(mynum,PFL_UAF) || plev(mynum) < LVL_WIZARD) {
        if ((a = vicf2(SPELL_VIOLENT, 7)) < 0) return;
    ...
    if (f) {
	    ...
	    strcpy(p.p_title, 
	           std_title(p.p_level, xtstbit(p.p_sflags, SFL_FEMALE)));
        }
        putuaf(&p);
    } else {
	setpsex(a, !psex(a));
...
static void change_level(void)
{
	...
	if (f) {
	    if (p.p_level <= LVL_WIZARD || new_lvl <= LVL_WIZARD) {
	        strcpy(p.p_title,
		     std_title(new_lvl, xtstbit(p.p_sflags, SFL_FEMALE)));
            }
	    p.p_level = new_lvl;
	    putuaf(&p);
	} else {
...
static void change_name(void)
{
	...
	if (isalpha(*wordbuf)) *wordbuf = toupper(*wordbuf);
...
void room_desc_handler( void * w, void * ad, int adlen)
{
	...
	*t = 0;
	...
	if (llong(loc) != NULL) free( llong(loc) );
	llong(loc) = s;
...
void change_desc(void)
{
	...
	if ( brkword() == -1) {
		...
		if (type == DESC_MOB && (loc = fpbn(wordbuf)) >= max_players) {

			change_mob_desc(loc, pname(loc));

		} else
		if (type == DESC_OBJ && (loc = fobn(wordbuf)) >= 0) {

			change_obj_desc(loc, oname(loc));
		}
		else
		if (type == DESC_PLR && getuaf(wordbuf, &p)) {
